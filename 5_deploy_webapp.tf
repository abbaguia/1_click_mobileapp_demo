resource "null_resource" "create-inventory" {
  ...
  provisioner "local-exec" {
    command = "sh /home/mail2maham/tensorflow-demo-sanjose-src/ansible/create_inventory_file.sh; echo ${oci_core_instance.this.*.public_ip[0]} >> /home/mail2maham/tensorflow-demo-sanjose-src/ansible/tf_inventory.yaml; echo ${oci_core_instance.this.*.public_ip[1]} >> /home/mail2maham/tensorflow-demo-sanjose-src/ansible/tf_inventory.yaml"
  }
}
...
resource "null_resource" "run-ansible-playbook" {
  ...
  # Create inventory
  provisioner "local-exec" {
    command = "sh /home/mail2maham/tensorflow-demo-sanjose-src/ansible/my-playbook.sh"
  }
}
